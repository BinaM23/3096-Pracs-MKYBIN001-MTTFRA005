...
int main(void)
{
  ...
  while (1)
  {
	 HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_8);
	  ...
	  int CCR = ADCtoCRR(ADCval);

	  //TASK 4
	  //Complete rest of implementation
	  ccr_val=CCR;
	  int dutyCycle=(CCR/47999.0) * 100;
	  sprintf(buffer,"ADC = %d\r\nCCR = %d\r\nDuty cycle = %d percent\r\n",ADCval,CCR,dutyCycle);
	  HAL_UART_Transmit(&huart2, buffer, sizeof(buffer), 1000);
	  HAL_UART_Transmit(&huart2, (uint8_t *)newline, 2, 10);
	  HAL_UART_Transmit(&huart2, (uint8_t *)newline, 2, 10);//print all vars

	  __HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_4, ccr_val); //adjusting duty cycle changes LED brightness
...
void EXTI0_1_IRQHandler(void)
{
	...
	int tic =HAL_GetTick(); //ensures exactly 500ms
	if(delay==500 && (tic>500)){
